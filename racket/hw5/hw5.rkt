  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([clsr (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clsr)
               (letrec ([clsr-env (closure-env clsr)]
                        [clsr-fun (closure-fun clsr)]
                        [env-with-arg (cons (cons (fun-formal clsr-fun) arg) clsr-env)]
                        [full-env (if (fun-nameopt clsr-fun)
                                      (cons (cons (fun-nameopt clsr-fun) clsr) env-with-arg)
                                      env-with-arg)])
                 (eval-under-env (fun-body clsr-fun) full-env))
  ...
  (fun #f "mapper"
       (fun "loop" "lst"
            (ifeq (isaunit (var "lst")) (int 1)
                  (aunit)
                  (apair (call (var "mapper") (fst (var "lst"))) (call (var "loop") (snd (var "lst"))))))))
