...
fun consume_string_literal (cs : char list) : string * char list =
  let
    fun go_to_closing (cs, in_literal) =
      case cs of
           #"\""::cs'   => (String.implode(rev_list in_literal), cs')
         | c::cs'       => go_to_closing(cs', c::in_literal)
         | _            => lexical_error "No closing double quote"
  in
    case cs of
         #"\""::cs'     => go_to_closing(cs', [])
       | _              => lexical_error "No string literal"
  end
...
fun consume_keyword (cs : char list) : token * char list =
  let
    val keyword_map = [("true", TrueTok), ("false", FalseTok), ("null", NullTok)]
    fun get_keyword_string(cs, acc) =
      case cs of
           []           => (String.implode(rev_list acc), [])
         | c::cs'       => if Char.isAlpha c
                           then get_keyword_string(cs', c::acc)
                           else (String.implode(rev_list acc), c::cs')
    val (keyword_str, tail) = get_keyword_string(cs, [])
  in
    case assoc(keyword_str, keyword_map) of
         SOME t => (t, tail)
       | NONE   => lexical_error "Invalid keyword"
  end
...
fun tokenize_char_list (cs : char list) : token list =
  case cs of
   ...
   | #"\t" :: cs        => tokenize_char_list cs (* ignore tabs *)
   | #" " :: cs        => tokenize_char_list cs (* ignore spaces *)
   ...
   | c :: cs =>
       ...
       then
         let
           val (keyword, cs) = consume_keyword (c :: cs)
         in
           keyword :: tokenize_char_list cs
         end
...
fun tokenize (s : string) : token list = tokenize_char_list(String.explode s)
...
fun parse_string (ts : token list) : string * token list =
  case ts of
       StringLit str :: ts'     => (str, ts')
     | _                        => syntax_error(ts, "No string literal")
...
fun expect (t : token, ts : token list) : token list =
  case ts of
       x :: ts' => if x = t
                   then ts'
                   else syntax_error(ts, "Expected: " ^ token_to_string t)
     | _        => syntax_error(ts, "Expected: " ^ token_to_string t)
...
fun parse_json (ts : token list) : json * token list =
  let
    fun parse_field_value (ts : token list) : (string * json) * token list =
      let
        val (field_name, ts) = parse_string(ts)
        val ts = expect(Colon, ts)
        val (json, ts) = parse_json(ts)
      in
        ((field_name, json), ts)
      end
    ...
    fun parse_field_value_list (ts : token list) : (string * json) list * token list =
      case ts of
           RBrace :: tail => ([], tail)
         | _ => let
                  val (pair, tail) = parse_field_value(ts)
                in
                  case tail of
                       Comma :: RBrace :: tail' => syntax_error(tail, "No elements after comma")
                     | Comma :: tail =>
                        let
                          val (pairs, tail) = parse_field_value_list(tail)
                        in (pair :: pairs, tail) end
                     | RBrace :: tail => ([pair], tail)
                     | _ => syntax_error(tail, "Unexpected token")
                end
    ...
    fun parse_array_element_list (ts : token list) : json list * token list =
      case ts of
           RBracket :: tail => ([], tail)
         | _ => let
                  val (j, tail) = parse_json(ts)
                in
                  case tail of
                       Comma :: RBracket :: tail' => syntax_error(tail, "No elements after comma")
                     | Comma :: tail =>
                        let
                          val (js, tail) = parse_array_element_list(tail)
                        in (j :: js, tail) end
                     | RBracket :: tail => ([j], tail)
                     | _ => syntax_error(tail, "Unexpected token")
                end
  in
    case ts of
       ...
       | LBrace :: ts           =>
           let
             val (pairs, ts) = parse_field_value_list(ts)
           in
             (Object pairs, ts)
           end
       | LBracket :: ts           =>
           let
             val (elements, ts) = parse_array_element_list(ts)
           in
             (Array elements, ts)
           end
